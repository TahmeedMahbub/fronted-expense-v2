...
// if (userLocationVar) {
//   const userLocation = L.map('userLocation', {
//     dragging: true // Make the map draggable
//   });
  
//   // Use geolocation API to get user's current location
//   navigator.geolocation.getCurrentPosition(function(position) {
//     const userLatLng = [position.coords.latitude, position.coords.longitude];
//     userLocation.setView(userLatLng, 18); // Set initial zoom level to 20 and center on user's location
    
//     const markerLocation = L.marker(userLatLng, {
//       draggable: true // Make the marker draggable
//     }).addTo(userLocation);
    
//     L.circle(userLatLng, { radius: 1500 }).addTo(userLocation); // Adjust radius as needed
//   }, function(error) {
//     console.error('Error getting user location:', error);
//     // Fallback to default location if user location cannot be retrieved
//     userLocation.setView([42.35, -71.08], 14);
//   });
  
//   L.tileLayer('https://{s}.tile.osm.org/{z}/{x}/{y}.png', {
//     attribution: 'Map data &copy; <a href="https://www.openstreetmap.org/">OpenStreetMap</a>',
//     maxZoom: 14 // Adjusted maxZoom to 14
//   }).addTo(userLocation);
    // }
...
//   navigator.geolocation.getCurrentPosition(function(position) {
//     const userLatLng = [position.coords.latitude, position.coords.longitude];
//     userLocation.setView(userLatLng, 20); // Set initial zoom level to 20 and center on user's location
...
//     fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${userLatLng[0]}&lon=${userLatLng[1]}`)
//       .then(response => response.json())
//       .then(data => {
//         const placeName = data.display_name;
//         markerLocation.bindPopup(`<b>${placeName}</b>`).openPopup();
//       })
//       .catch(error => {
...
if (userLocationVar) {
  ...
  navigator.geolocation.getCurrentPosition(function(position) {
    const userLatLng = [position.coords.latitude, position.coords.longitude];
    userLocation.setView(userLatLng, 30); // Set initial zoom level to 20 and center on user's location
    ...
    markerLocation.on('dragend', function(event) {
      ...
      // Fetch place name using reverse geocoding for the new marker location
      fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${newLatLng.lat}&lon=${newLatLng.lng}`)
        .then(response => response.json())
        .then(data => {
          const placeName = data.display_name;
          $("#location_input").val(placeName);
        })
        .catch(error => {
          console.error('Error fetching place name:', error);
        });
    ...
    // Fetch place name using reverse geocoding for the initial marker location
    fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${userLatLng[0]}&lon=${userLatLng[1]}`)
      .then(response => response.json())
      .then(data => {
        const placeName = data.display_name;
    ...
    L.circle(userLatLng, { radius: 300 }).addTo(userLocation); // Adjust radius as needed
  }, function(error) {
    console.error('Error getting user location:', error);
    // Fallback to default location if user location cannot be retrieved
    userLocation.setView([42.35, -71.08], 16);
  });
  
  L.tileLayer('https://{s}.tile.osm.org/{z}/{x}/{y}.png', {
    attribution: '',
    maxZoom: 16 // Adjusted maxZoom to 14
  }).addTo(userLocation);
}
